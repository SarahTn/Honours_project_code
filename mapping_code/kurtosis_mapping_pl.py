...
def save_kurt_map(in_dir, ROOT_DIR, bval_cutoff, vox):
    
    # Call next function
    Dappmap, Kappmap, rmse_map, aic_map = func_kurtmapping(in_dir, bval_cutoff, vox)
    ...
    # Save maps
    sitk.WriteImage(Dapp_map_image, join(out_path,'Dapp_map_kurtosis_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
    sitk.WriteImage(Kappmap_map_image, join(out_path, 'Kapp_map_kurtosis_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
    sitk.WriteImage(rmse_map_image, join(out_path, 'rmse_map_kurtosis_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
    sitk.WriteImage(aic_map_image, join(out_path, 'aic_map_kurtosis_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
    print(out_path)

#%%
def func_kurtmapping(in_dir, bval_cutoff, vox):
...
def fit_kurt_in_parallel(S, bvals_tot, bval_cutoff, vox):
    ...
    fit_model = Model(models.kurtosis, nan_policy='propagate')
...
def fit_slice_kurt(S, bvals_tot, bval_cutoff, vox, fit_model, start_Dapp):
    ...
    for v in vox_idx:
          Dappslice[v], Kappslice[v], rmse_slice[v], aic_slice[v] = fit_voxels_kurtosis(S_z[:,v], bvals_tot, bval_cutoff, fit_model, start_Dapp)
