...
def save_seg_ivim_maps(in_dir, ROOT_DIR, bval_cutoff, vox):
    '''
    bval_cutoff: b-value threshold for fitting
    vox: tuple of voxels inside the prostate
    '''
    D_map, Dstar_map, f_map, rmse_map, aic_map = func_seg_ivim_mapping(in_dir, bval_cutoff, vox)
    ...
    # D_ci_map_image = sitk.GetImageFromArray(D_ci_map)
    # D_ci_map_image.CopyInformation(adc_image1)
    ...
    # Dstar_ci_map_image = sitk.GetImageFromArray(Dstar_ci_map)
    # Dstar_ci_map_image.CopyInformation(adc_image1)
    ...
    # f_ci_map_image = sitk.GetImageFromArray(f_ci_map)
    # f_ci_map_image.CopyInformation(adc_image1)
    ...
    # Save maps
    sitk.WriteImage(D_map_image, join(out_path,'D_map_seg_ivim_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
    sitk.WriteImage(Dstar_map_image, join(out_path, 'Dstar_map_seg_ivim_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
    sitk.WriteImage(f_map_image, join(out_path,'f_map_seg_ivim_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))   
    sitk.WriteImage(rmse_map_image, join(out_path,'rmse_map_seg_ivim_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
    sitk.WriteImage(aic_map_image, join(out_path,'aic_map_seg_ivim_bval_cutoff_{}.nii.gz'.format(bval_cutoff)))
...
#%%
def func_seg_ivim_mapping(in_dir, bval_cutoff, vox):
    ...
    # Fit maps
    D_map, Dstar_map, f_map, rmse_map, aic_map = fit_seg_ivim(dwi_afterdenoising, bvals1, bval_cutoff, vox)
    
    return D_map, Dstar_map, f_map, rmse_map, aic_map
#%%
def fit_seg_ivim(S, bvals, bval_cutoff, vox):
    ''' 
    ...
    out = [Parallel(n_jobs=-1, verbose=20)(delayed(fit_slice_seg_ivim)(S[:,sl,:,:], bvals, bval_cutoff, np.squeeze(vox_arr[1:, list(np.where(vox[0] == sl))]), D_model, ivim_model, init_D) for sl in slices)]
    ...
    # D_ci_map = np.zeros(shape=dim[1:])
    Dstar_map = np.zeros(shape=dim[1:])
    # Dstar_ci_map = np.zeros(shape=dim[1:])
...
#%%
def fit_slice_seg_ivim(S, bvals, bval_cutoff, vox, D_model, ivim_model, init_D):
    ...
    rmse_vals = np.zeros(dim[1]*dim[2])
    aic_vals = np.zeros(dim[1]*dim[2])
    ...
    for v in vox_idx:
          Dslice[v], Dstarslice[v], fslice[v], rmse_vals[v], aic_vals[v] = fit_voxels_seg_ivim(S_z[:,v], bvals, D_model, ivim_model, bval_cutoff, init_D)
    
    return np.reshape(Dslice,(dim[1],dim[2])), np.reshape(Dstarslice,(dim[1],dim[2])), np.reshape(fslice,(dim[1],dim[2])), np.reshape(rmse_vals,(dim[1],dim[2])), np.reshape(aic_vals,(dim[1],dim[2]))
...
def fit_voxels_seg_ivim(signals, bvals, D_model, ivim_model, bval_cutoff, init_D):
    ...
    # upper_bound = CI_1['m'][2][1]
    # if math.isinf(upper_bound):
    #     D_ci = math.nan
    # else:
    #     D_ci = 2*(upper_bound - D)    # upper bound - D
    # D_ci = 0
...
# # # IVIM mapping test
# bval_cutoff = 200
# D_map, Dstar_map, f_map, rmse_map, aic_map = func_seg_ivim_mapping(in_dir, bval_cutoff, vox_tuple)

# cmap = plt.get_cmap('viridis')
# plt.rcParams.update({'font.size': 13})
# plt.imshow(rmse_map[8], cmap=cmap)
# plt.colorbar(label='$10^{-6}$ $mm^2/s$')
# plt.title('RMSE map seg IVIM')

# cmap = plt.get_cmap('viridis')
# plt.rcParams.update({'font.size': 13})
# plt.imshow(aic_map[8], cmap=cmap)
# plt.colorbar(label='$10^{-6}$ $mm^2/s$')
# plt.title('AIC map seg IVIM')

# cmap = plt.get_cmap('viridis')
# plt.rcParams.update({'font.size': 13})
# plt.imshow(Dstar_map[8], cmap=cmap)
# plt.colorbar(label='$10^{-6}$ $mm^2/s$')
# plt.title('Dstar map seg IVIM')
...
# plt.colorbar(label='$10^{-6}$ $mm^2/s$')
# plt.title('D map seg b-cutoff {}'.format(bval_cutoff))
...
# plt.colorbar(label='$10^{-6}$ $mm^2/s$')
# plt.title('Dstar map seg b-cutoff {}'.format(bval_cutoff))
...
# plt.colorbar(label='$10^{-6}$ $mm^2/s$')
# plt.title('f map seg b-cutoff {}'.format(bval_cutoff))
...
# # Create empty arrays to store x,y but in one dimension
# Dslice = np.zeros(dim[1]*dim[2]) 
# D_ci = np.zeros(dim[1]*dim[2])
# Dstarslice = np.zeros(dim[1]*dim[2])
# Dstar_ci = np.zeros(dim[1]*dim[2])
# fslice = np.zeros(dim[1]*dim[2])
# f_ci = np.zeros(dim[1]*dim[2])
# chisqr_vals = np.zeros(dim[1]*dim[2])
...
# vox_idx = np.ravel_multi_index((vox[0,:],vox[1,:]), (dim[1],dim[2]))
# for v in vox_idx:
#       Dslice[v], Dstarslice[v], fslice[v], chisqr_vals[v], D_ci[v], Dstar_ci[v], f_ci[v] = fit_voxels_seg_ivim(S_z[:,v], bvals1, D_model, ivim_model, bval_cutoff, init_D)
       
       
       
# # # Plot map
# Dstar_ci = np.reshape(Dstar_ci,(dim[1],dim[2]))
# # flip_ivim_map = [np.flipud(ivim_map[sl,:,:]) for sl in range(ivim_map.shape[0])]
# cmap = plt.get_cmap('magma')
# plt.rcParams.update({'font.size': 13})
# plt.imshow(Dstar_ci, cmap=cmap)
# plt.colorbar(label='$10^{-6}$ $mm^2/s$')
# plt.title('D CI map')
#%%
...
#     ivim_model = Model(models.ivim, nan_policy='propagate')
#     D_out, Dstar_out, f_out, result  = fit_voxels_seg_ivim(signals, bvals1, linear_model, ivim_model, bval_cutoff, init_D)
#     params.append([ D_out, Dstar_out, f_out])
#     CI = result.conf_interval(sigmas=[2])
#     print(result.ci_report(ndigits=5)) 
    
#     stats = result.fit_report()
#     # print(CI)
    
#     plt.scatter(bvals1, signals)
#     plt.plot(bvals1, result.best_fit, '-', label='best fit: {}'.format(labels[j]))
